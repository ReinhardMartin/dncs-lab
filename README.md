...
The commands used to configure the network are listed and explained below:

- assign an IP address to a specifc interface

        sudo ip addr add [ip] dev [interface]

- set the interface in the "up" status
...
- redirect the traffic from a subnet to an IP address

        sudo ip route add [subnets addresses] via [ip] 
       
- create a VLAN and assign a tag
        
        sudo ip link add link [interface] name [VLAN] type vlan id [tag]
        
- create a bridge and assign the VLAN tag to the interfaces

        ovs-vsctl add-br switch
        ovs-vsctl add-port switch [interface]
        ovs-vsctl add-port switch [interface] tag="[tag]"
        
- install docker and test docker image and run them
...
## Test result
First we create the network with the command 'vagrant up', then we can log in each VM via 'vagrant ssh [VM-name]' and check the reachability of the other VMs with command 'ping -c3 [url]'.
By logging in either host-a or host-b and making a request to host-c with the command 'curl 192.168.4.2' we receive the following HTML file:
...
<p><span>Server&nbsp;name:</span> <span>091638f13cf8</span></p>
<p class="smaller"><span>Date:</span> <span>11/Oct/2021:15:32:20 +0000</span></p>
...
<div class="check"><input type="checkbox" id="check" onchange="changeCookie()"> Auto Refresh</div>
    <div id="footer">
        <div id="center" align="center">
            Request ID: f9dc151802ca3a764aab8628cd66f081<br/>
            &copy; NGINX, Inc. 2018
        </div>
    </div>
</body>
